...
class Clicker {
	...
	public HashMap<Integer, int[]> clickeds = new HashMap<>();
	 ...
	 * @param config the {@link Solo.Config} instance.
	 * @param context the start {@link Activity}.
	 */

	public Clicker(Solo.Config config, ActivityUtils activityUtils, ViewFetcher viewFetcher, Sender sender, Instrumentation inst, Sleeper sleeper, Waiter waiter, WebUtils webUtils, DialogUtils dialogUtils, Activity context) {
		...
		this.screenshotTaker = new ScreenshotTaker(config, inst, activityUtils, viewFetcher, sleeper);
		this.config = config;
	 ...
	 * @param y the y coordinate
	 */
	public void clickOnScreenForWeb(float x, float y) {
		click(x, y);
	}
	...
	public void clickLongOnScreen(float x, float y, int time, View view) {
		...
		if(!successfull) {
			Log.w(Solo.LOG_TAG, "Long click at ("+x+", "+y+") can not be completed! ("+(ex != null ? ex.getClass().getName()+": "+ex.getMessage() : "null")+")");
		}
	...
	public void clickLongOnTextAndPress(String text, int index)
	{
		...
		try{
			...
			Log.e(Solo.LOG_TAG, "Can not press the context menu!");
		}
	...
	private void openMenu(){
		...
		if(!dialogUtils.waitForDialogToOpen(MINI_WAIT, false)) {
			try{
				...
				Log.e(Solo.LOG_TAG, "Can not open the menu!");
			}
		}
	}
	...
	public void clickOnActionBarHomeButton() {
		...
		try {
			...
			Log.d(Solo.LOG_TAG, "Can not find methods to invoke Home button!");
		}
	...
	public void clickOnWebElement(By by, int match, boolean scroll, boolean useJavaScriptToClick){
		...
		if(useJavaScriptToClick){
			...
			if(webElement == null){
				Log.e(Solo.LOG_TAG, "WebElement with " + webUtils.splitNameByUpperCase(by.getClass().getSimpleName()) + ": '" + by.getValue() + "' is not found!");
			}
		...
		if(webElementToClick == null){
			if(match > 1) {
				Log.e(Solo.LOG_TAG, match + " WebElements with " + webUtils.splitNameByUpperCase(by.getClass().getSimpleName()) + ": '" + by.getValue() + "' are not found!");
			}
			else {
				Log.e(Solo.LOG_TAG, "WebElement with " + webUtils.splitNameByUpperCase(by.getClass().getSimpleName()) + ": '" + by.getValue() + "' is not found!");
			}
	...
	public void clickOnText(String regex, boolean longClick, int match, boolean scroll, int time) {
		...
		if (textToClick != null) {
			...
			if(match > 1){
				Log.e(Solo.LOG_TAG, match + " matches of text string: '" + regex +  "' are not found!");
			}
				...
				for (TextView textView : allTextViews) {
					Log.d(Solo.LOG_TAG, "'" + regex + "' not found. Have found: '" + textView.getText() + "'");
				}
				allTextViews = null;
				Log.e(Solo.LOG_TAG, "Text string: '" + regex + "' is not found!");
			}
		}
	}
	...
	public <T extends TextView> void clickOn(Class<T> viewClass, String nameRegex) {
		...
		if (viewToClick != null) {
			...
			for (T view : allTextViews) {
				Log.d(Solo.LOG_TAG, "'" + nameRegex + "' not found. Have found: '" + view.getText() + "'");
			}
			Log.e(Solo.LOG_TAG, viewClass.getSimpleName() + " with text: '" + nameRegex + "' is not found!");
		}
	}
	...
	public ArrayList<TextView> clickInList(int line, int index, int id, boolean longClick, int time) {
		...
		if(absListView == null){
			Log.e(Solo.LOG_TAG, "AbsListView is null!");
	...
	public ArrayList<TextView> clickInRecyclerView(ViewGroup recyclerView, int line) {
	...
	public void clickInRecyclerView(ViewGroup recyclerView, int itemIndex, int id) {
		clickInRecyclerView(recyclerView, itemIndex, 0, id, false, 0);
	}
	...
	public ArrayList<TextView> clickInRecyclerView(ViewGroup recyclerView, int itemIndex, int recyclerViewIndex, int id, boolean longClick, int time) {
		View viewOnLine = null;
		final long endTime = SystemClock.uptimeMillis() + Timeout.getSmallTimeout();
	...
	private void failIfIndexHigherThenChildCount(ViewGroup viewGroup, int index, long endTime){
		while(index > viewGroup.getChildCount()){
			...
			if (timedOut){
				...
				Log.e(Solo.LOG_TAG, "Can not click on index " + index + " as there are only " + numberOfIndexes + " indexes available");
				break;
			}
			sleeper.sleep();
		}
	...
	private View getViewOnAbsListLine(AbsListView absListView, int index, int lineIndex){
		...
		while(view == null){
			final boolean timedOut = SystemClock.uptimeMillis() > endTime;
			if (timedOut){
				Log.e(Solo.LOG_TAG, "View is null and can therefore not be clicked!");
				break;
			}
